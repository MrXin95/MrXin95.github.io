...
function updateLatLngInnerHtmlInvalidCoords() {
    let x = shadow.querySelector(".location-set-display");
    x.style.visibility = "visible"
    x.innerHTML = `Try to enter valid coordinates for example: 30.26390074260293, -239.88225549459457`;
}
...
class MapPickLocation extends HTMLElement {
    ...
    addLatLngInputs() {
        ...
        numBlock.className = 'block-num';

        const latElemLabel = document.createElement('label');
        latElemLabel.className = "GPS-label";
        latElemLabel.for = 'latitude';
        latElemLabel.innerHTML = '纬&emsp;&emsp;度:';

        const lngElemLabel = document.createElement('label');
        lngElemLabel.className = "GPS-label";
        lngElemLabel.for = 'longitude';
        lngElemLabel.innerHTML = '经&emsp;&emsp;度:'
        ...
        addCoordsName.innerHTML = '增加一个位置';
